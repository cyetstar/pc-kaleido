...
<template>
  <section class="h-page-section" ref="appManagePage">
    ...
    <a-space class="h-btn-space">
      <h-button v-permissKey="'movieActor:create'" type="primary" @click="onCreateRecord">新增</h-button>
      <h-button-delete v-permissKey="'movieActor:delete'" plain :disabled="selectedRowKeys.length === 0"
                       @delete="onDeleteRecord(selectedRowKeys)"/>
      <h-button v-permissKey="'movieActor:export'" @click="onShowExportRecord">导出</h-button>
    </a-space>
    ...
    <h-table-data ref="tableRef" row-key="id" @loadData="onLoadData" @selection-change="onSelectionChange">
      ...
      <a-table-column title="操作" align="center" width="300px">
        <template #="{ record }">
          <a-space :size="0">
            <h-button type="primary" size="small" link v-permissKey="'movieActor:view'"
                      @click="onViewRecord(record.id)">详情</h-button>
            <h-button type="primary" size="small" link v-permissKey="'movieActor:update'"
                      @click="onUpdateRecord(record.id)">编辑</h-button>
            <h-button-delete size="small" link v-permissKey="'movieActor:delete'"
    ...
    <movieActorForm ref="formRef" @save-complete="onSaveComplete"></movieActorForm>
...
import {apiMovieActorPage, apiMovieActorDelete, apiMovieActorColumn, apiMovieActorExport} from "@/api/movie/movieActorApi.ts"
...
const onLoadData = (params, callbacks) => {
  callbacks(apiMovieActorPage({ ...params, ...searchForm.value, searchCount: true }));
};
...
const onDeleteRecord = (recordId) => {
  ...
  apiMovieActorDelete(id).then(res => {
    if (res) {
      tableRef.value.load();
...
const onLoadColumn = (callbacks) => {
  callbacks(apiMovieActorColumn);
};

const onExport = async (params) => {
  const data = await apiMovieActorExport({
    ...params,
    ...searchForm.value,
  });
