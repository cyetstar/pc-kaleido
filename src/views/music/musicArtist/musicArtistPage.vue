...
<template>
  <section class="h-page-section" ref="appManagePage">
    ...
    >
      ...
      <h-col :span="6">
        <h-input label="名称" v-model:value="searchForm.mc" name="mc" />
      </h-col>
      <h-col :span="6">
        <h-select
          label="艺术家类型"
          dict-type="ysjlx"
          v-model:value="searchForm.ysjlx"
          name="ysjlx"
        />
      </h-col>
    </h-form-search>
...
import {
  musicArtistPageApi,
  musicArtistDeleteApi,
  musicArtistColumnApi,
  musicArtistExportApi,
...
const loadData = (params, callbacks) => {
  callbacks(
    musicArtistPageApi({ ...params, ...searchForm.value, searchCount: true })
  );
};
