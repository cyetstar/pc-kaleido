...
<template>
  <section class="h-page-section" ref="appManagePage">
    <h-form-search v-model:form="searchForm" @search="onSearch" @reset="onReset">
        ...
        <h-col :span="6">
          <h-select-datetime label="创建时间" type="datetime"
                             v-model:value="searchForm.cjsj" name="cjsj"/>
        </h-col>
        <h-col :span="6">
          <h-select-datetime label="修改时间" type="datetime"
                             v-model:value="searchForm.xgsj" name="xgsj"/>
        </h-col>
    </h-form-search>
    ...
    <h-table-data ref="tableRef" row-key="id" @loadData="onLoadData" @selection-change="onSelectionChange">
        ...
        <a-table-column title="MusicBrainz编号" data-index="musicbrainzId" align="center"></a-table-column>
        <a-table-column title="Plex编号" data-index="plexId" align="center"></a-table-column>
        <a-table-column title="创建时间" data-index="cjsj" align="center"></a-table-column>
        <a-table-column title="修改时间" data-index="xgsj" align="center"></a-table-column>
        <a-table-column title="Plex缩略图" data-index="plexThumb" align="center"></a-table-column>
        <a-table-column title="网易云音乐编号" data-index="neteaseId" align="center"></a-table-column>
    ...
    <musicArtistForm ref="formRef" @save-complete="onSaveComplete"></musicArtistForm>
...
import {apiMusicArtistPage, apiMusicArtistDelete, apiMusicArtistColumn, apiMusicArtistExport} from "@/api/music/musicArtistApi.ts"
...
const onLoadData = (params, callbacks) => {
  callbacks(apiMusicArtistPage({ ...params, ...searchForm.value, searchCount: true }));
};
