...
<template>
  <section class="h-page-section" ref="appManagePage">
    ...
    <a-space class="h-btn-space">
      <h-button v-permissKey="'comicAuthor:create'" type="primary" @click="onCreateRecord">新增</h-button>
      <h-button-delete v-permissKey="'comicAuthor:delete'" plain :disabled="selectedRowKeys.length === 0"
                       @delete="onDeleteRecord(selectedRowKeys)"/>
      <h-button v-permissKey="'comicAuthor:export'" @click="onShowExportRecord">导出</h-button>
    </a-space>
    ...
    <h-table-data ref="tableRef" row-key="id" @loadData="onLoadData" @selection-change="onSelectionChange">
      ...
      <a-table-column title="操作" align="center" width="300px">
        <template #="{ record }">
          <a-space :size="0">
            <h-button type="primary" size="small" link v-permissKey="'comicAuthor:view'"
                      @click="onViewRecord(record.id)">详情</h-button>
            <h-button type="primary" size="small" link v-permissKey="'comicAuthor:update'"
                      @click="onUpdateRecord(record.id)">编辑</h-button>
            <h-button-delete size="small" link v-permissKey="'comicAuthor:delete'"
    ...
    <comicAuthorForm ref="formRef" @save-complete="onSaveComplete"></comicAuthorForm>
...
<script setup>
import comicAuthorForm from './comicAuthorForm.vue';
import { ref, getCurrentInstance } from 'vue';
import {apiComicAuthorPage, apiComicAuthorDelete, apiComicAuthorColumn, apiComicAuthorExport} from "@/api/comic/comicAuthorApi.ts"
...
const onLoadData = (params, callbacks) => {
  callbacks(apiComicAuthorPage({ ...params, ...searchForm.value, searchCount: true }));
};
...
const onViewRecord = (id) => {
  router.push({ path: "comic/comicAuthor/view", query: { id } })
}
...
const onDeleteRecord = (recordId) => {
  ...
  apiComicAuthorDelete(id).then(res => {
    if (res) {
      tableRef.value.load();
...
const onLoadColumn = (callbacks) => {
  callbacks(apiComicAuthorColumn);
};

const onExport = async (params) => {
  const data = await apiComicAuthorExport({
    ...params,
    ...searchForm.value,
  });
